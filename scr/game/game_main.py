...
PATH = "D:/Install/Projects/Python/Arkanoid_socket_game/scr/"
...
class Game:
    ...
    def __init__(self):
        ...
        while True:
            ...
            if self.player_number == self.FIRST_PLAYER:
            ...
            self.generator = ObstaclesGenerator(self.screen, self.obstacle_sprites, self.obstacle_hitted_sprites,
                                                self.ball,
    ...
    def process_data_from_server(self):
        ...
        while len(self.messages) > 0:
            ...
            if event.type == self.EVENT_PLATFORM_POSITION:
                if event.data.count(ServerMessage.DATA_DIVIDER) == 1:
                    self.platform_opp.position = tuple(map(float, event.data.split(ServerMessage.DATA_DIVIDER)))

            elif event.type == self.EVENT_OBSTACLE_CREATION:
                if event.data.count(ServerMessage.DATA_DIVIDER) == 2:
                    x, y, start_lives = event.data.split(ServerMessage.DATA_DIVIDER)
                ...
                if event.data.count(ServerMessage.DATA_DIVIDER) == 1:
                    self.ball.position = tuple(map(float, event.data.split(ServerMessage.DATA_DIVIDER)))
